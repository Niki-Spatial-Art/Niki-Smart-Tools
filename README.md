...
当前项目没有部署在阿里云上，也没有使用阿里云数据库、ECS、函数计算或对象存储。
...
- 用云数据库或 MongoDB 类能力统一保存动作卡、复盘文本和公告摘要。
- 用向量检索把历史动作卡、错过样本和每日维护记录做成可问答的交易记忆库。
- 继续保留“真实下单必须人工确认”的闸门。
...
公开仓库不会提供 iFind token。别人不能使用你的 iFind 接口，每个使用者都必须配置自己的 iFind 账号和接口权限。
