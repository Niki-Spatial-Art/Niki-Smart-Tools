...
try {
    ...
    $env:XINGYAO_CACHE_PATH = "data\xingyao_option_basic_cache.json"

    python -c "import monitor; r = monitor.fetch_xingyao_option_basic_rows(); print('Xingyao source:', r.get('source')); print('Xingyao contracts:', r.get('contract_count', 0)); print('Xingyao cache:', r.get('cache_path') or 'data/xingyao_option_basic_cache.json'); raise SystemExit(0 if r.get('contract_count', 0) else 1)"
}
finally {
